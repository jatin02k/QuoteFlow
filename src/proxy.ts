...
export async function proxy(request: NextRequest) {
  ...
  const isRespondPage = request.nextUrl.pathname.startsWith('/respond')
  const isProtectedWorkspace = 
    request.nextUrl.pathname.startsWith('/dashboard') ||
    request.nextUrl.pathname.startsWith('/rfqs') ||
    request.nextUrl.pathname.startsWith('/vendors') ||
    request.nextUrl.pathname.startsWith('/settings')

  // Public vendor response page explicitly bypasses auth
  if (isRespondPage) {
